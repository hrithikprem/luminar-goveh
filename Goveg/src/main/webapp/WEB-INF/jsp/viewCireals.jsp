...
<tbody>
					<c:set var="i" value="0" />
					<c:forEach items="${cireal}" var="cireal" varStatus="loop">
					    <c:if test="${cireal.catId==4 }">
					        <tr>
					            <td>${i+1}</td>
					            <td>${fn:toUpperCase(cireal.itemName)}</td>
					            <td>${cireal.itemPrice}</td>
					            <td>${cireal.intemQty}</td>
					        </tr>
